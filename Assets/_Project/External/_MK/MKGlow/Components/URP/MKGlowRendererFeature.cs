    ...
    public class MKGlowRendererFeature : ScriptableRendererFeature, MK.Glow.ISettings
    {
        ...
        class MKGlowRenderPass : ScriptableRenderPass, MK.Glow.ICameraData
        {
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                void CopySource()
                {
                    bool xrInstancingEnabled = GetStereoEnabled() && PipelineProperties.singlePassStereoInstancedEnabled;
                    if(Compatibility.copyTextureSupported && !xrInstancingEnabled)
                    {
                        cmd.CopyTexture(sourceRenderTarget.renderTargetIdentifier, destinationRenderTarget.renderTargetIdentifier);
                    }
                    else
                    {
                        cmd.Blit(xrInstancingEnabled ? BuiltinRenderTextureType.CameraTarget : sourceRenderTarget.renderTargetIdentifier, destinationRenderTarget.renderTargetIdentifier);
                    }
                }
